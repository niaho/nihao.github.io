...
{
	int i;
   sm4_setkey( ctx->ek, key );
   sm4_setkey(ctx->dk, key);
   for (i = 0; i < 16; i++)
   {
	   SWAP(ctx->dk[i], ctx->dk[31 - i]);
   }
}
                   ...
                   unsigned char *output)
{
    while( length > 0 )
    {
        sm4_one_round( ctx->ek, input, output );
        input  += 16;
        output += 16;
        length -= 16;
    }
...
{
	int i;
	unsigned char *mask = iv;
	while (length > 0)
	{
		for (i = 0; i < 16; i++)
			buff[i] ^= mask[i];
		sm4_one_round(ctx->ek, buff, buff);
		mask = buff;
		buff += 16;
		length -= 16;
	}
				 ...
				 int  length)
{
	...
	{
		memcpy(temp, buff, 16);
		sm4_one_round(ctx->dk, buff, buff);
		for (i = 0; i < 16; i++)
			buff[i] ^= iv1[i];
		memcpy(iv1, temp, 16);
		buff += 16;
                    ...
                    unsigned char *output )
{
    ...
    if( mode == SM4_ENCRYPT )
    {
        while( length > 0 )
        {
            ...
            sm4_one_round( ctx->ek, output, output );
            memcpy( iv, output, 16 );
    ...
    else /* SM4_DECRYPT */
    {
        while( length > 0 )
        {
            ...
            sm4_one_round( ctx->dk, input, output );
