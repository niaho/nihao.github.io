...
static OraText *user=(OraText *)"SYSDBA";
static OraText *password=(OraText *)"SYSDBA";
...
static sb4 alloc_desc(OCIDateTime *dvar[], OCIInterval *ivar[], sword size1, sword size2 );
static void chk_err( OCIError *errhp, boolean ec, sb2 linenum, OraText *comment );
...
static sb4 exe_insert(OraText *insstmt, OraText *strdate[COLNUM-1], 
   OraText *fmtdate[3], OCIDateTime *dvar[], OCIInterval *ivar[], size_t rownum);
static sb4 exe_sql(OraText *stmt);
static sb4 free_desc(OCIDateTime *dvar[], OCIInterval *ivar[], sword size1, sword size2 );
...
int main(int argc,char *argv[])
{
 ...
 strcpy((char *)cstring, "localhost:5139/system");
...
/*--------------------------------------------------------*/
sb4 add_interval(OraText *dt, OraText *interval, OraText *fmt, ub4 type1, ub4 type2)
{
...
/* Allocate descriptors */
/*--------------------------------------------------------*/
sb4 alloc_desc(OCIDateTime *dvar[], OCIInterval *ivar[], sword size1, sword size2)
{
 sword i;
...
/* Error handling  */
/*--------------------------------------------------------*/
void chk_err(OCIError *errhp, boolean ec, sb2 linenum, OraText *comment)
{
  OraText  msgbuf[1024];
  sb4 errcode;
...
/*--------------------------------------------------------*/
sb4 conv_dt(OraText *str1, ub4 type1, ub4 type2, OraText *fmt1,
	OraText *fmt2, OraText *name1, OraText *name2)
{
...
/*--------------------------------------------------------*/
sb4 exe_insert(OraText *insstmt, OraText *strdate[COLNUM - 1],
	OraText *fmtdate[3], OCIDateTime *dvar[], OCIInterval *ivar[], size_t rownum)
{
 sword i;
 size_t colc = rownum;
...
/* Free the descriptors */
/*--------------------------------------------------------*/
sb4 free_desc(OCIDateTime *dvar[], OCIInterval *ivar[], sword size1, sword size2)
{
 sword i;
