...
/* ----------------------------------------------------------------- */
sword init_handles(OCIEnv **envhp,OCISvcCtx **svchp,OCIError **errhp,OCIServer **srvhp,OCISession **authp,ub4 init_mode)
{
...
/* ----------------------------------------------------------------- */
sword attach_server(ub4 mode,OCIServer *srvhp,OCIError *errhp,OCISvcCtx *svchp)
{
  text *cstring = (text *)"localhost:5139/system";
...
sword alloc_bind_handle(OCIStmt *stmthp,OCIBind *bndhp[],int  nbinds)
{
  int  i;
  /*
...
/*---------------------------------------------------------------------*/
sword finish_demo(boolean loggedon,OCIEnv *envhp,OCISvcCtx *svchp,OCIServer *srvhp,OCIError *errhp,OCISession *authp,OCIStmt *stmthp,text *userid)
{
...
/* ----------------------------------------------------------------- */
sword bind_in_name(OCIStmt *stmthp,OCIBind *bndhp[],OCIError *errhp)
{
...
/* ----------------------------------------------------------------- */
sword bind_name(OCIStmt *stmthp,OCIBind *bndhp[],OCIError *errhp)
{
...
/* ----------------------------------------------------------------- */
sword bind_array(OCIBind *bndhp[],OCIError *errhp)
{
...
/* ----------------------------------------------------------------- */
sword bind_dynamic(OCIBind *bndhp[],OCIError *errhp)
{
  int  i;
...
/* ----------------------------------------------------------------- */
sword bind_input(OCIStmt *stmthp,OCIBind *bndhp[],OCIError *errhp)
{
...
/* ----------------------------------------------------------------- */
sword bind_output(OCIStmt *stmthp,OCIBind *bndhp[],OCIError *errhp)
{
...
/* ----------------------------------------------------------------- */
sword bind_low_high(OCIStmt *stmthp,OCIBind *bndhp[],OCIError *errhp)
{
...
/* ----------------------------------------------------------------- */
sword demo_insert(OCISvcCtx *svchp,OCIStmt *stmthp,OCIBind *bndhp[],OCIError *errhp)
{
  int  i, j;
  int  num[MAXITER];
...
/* ----------------------------------------------------------------- */
sword demo_update(OCISvcCtx *svchp,OCIStmt *stmthp,OCIBind *bndhp[],OCIError *errhp)
{
  int    i, j;
  int    range_size = 3;
...
/* ----------------------------------------------------------------- */
sword demo_delete(OCISvcCtx *svchp,OCIStmt *stmthp,OCIBind *bndhp[],OCIError *errhp)
{
  int    i, range_size = 3;
  sword  retval;
...
/* ----------------------------------------------------------------- */
sb4 cbf_no_data(dvoid *ctxp,OCIBind *bindp,	ub4 iter, ub4 index,dvoid **bufpp,ub4 *alenpp,ub1 *piecep,dvoid **indpp)
{
  *bufpp = (dvoid *) 0;
  *alenpp = 0;
   null_ind = -1;
...
/* ----------------------------------------------------------------- */
sb4 cbf_get_data(dvoid *ctxp, OCIBind *bindp, ub4 iter, ub4 index, dvoid **bufpp,ub4 **alenp,ub1 *piecep,dvoid **indpp,ub2 **rcodepp)
{
  static ub4  bufl = 0;
   ub4  rows = 0;
  ub4    pos = *((ub4 *)ctxp);
  ...
  if (index == 0)
  {
    (void) OCIAttrGet(( dvoid *) bindp, OCI_HTYPE_BIND, (dvoid *)&rows,
...
/* allocate locators.                                                */
/* ----------------------------------------------------------------- */
sword locator_alloc(OCILobLocator *lob[],ub4 rows)
{
  ub4 i;
...
/* free locators.                                                    */
/* ----------------------------------------------------------------- */
sword locator_free(OCILobLocator *lob[],ub4 rows)
{
  ub4 i;
...
/* ----------------------------------------------------------------- */
sword select_locator(OCISvcCtx *svchp,OCIStmt *stmthp,OCIBind *bndhp[],OCIError *errhp)
{
  OCIDefine *dfnhp[3];
...
/* ----------------------------------------------------------------- */
sword check_lob(int iters,OCISvcCtx *svchp)
{
  int i, j;
...
/* ----------------------------------------------------------------- */
sword read_piece(OCISvcCtx *svchp,OCILobLocator *lob,ub2 lobtype)
{
  ub1  buf[MAXBUFLEN];
  ub4  amt = MAXBUFLEN;
  ub4  offset = 1;
...
/* ----------------------------------------------------------------- */
sword write_piece(OCISvcCtx *svchp,OCILobLocator *lob,ub2 lobtype)
{
  ub1  buf[MAXBUFLEN];
  ub4  amt = MAXBUFLEN;
  ub4  offset = 1;
