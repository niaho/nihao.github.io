#ifndef __KCI_DEF_H__
#define __KCI_DEF_H__
#ifndef ORATYPES
# define ORATYPES
# define SX_ORACLE
# define SX3_ORACLE 
...
typedef unsigned char   KunText;

#define KCI_NUMBER_SIZE 22
#ifndef KUTYXTT3_ORACLE
struct KCINumber
{
	ub1 KCINumberPart[KCI_NUMBER_SIZE];
};
typedef struct KCINumber KCINumber;
#define ORL_KCINUMBER_DEFINED
#endif

typedef ub2 KCIDuration;
...
typedef struct KCITime KCITime;

/*
* KCITime - KCI TiMe portion of date
*
* This structure should be treated as an opaque structure as the format
* of this structure may change. Use KCIDateGetTime/KCIDateSetTime
* to manipulate time portion of KCIDate.
*/
...
struct KCIDate
{
	...
	KCITime KCIDateTime;                                               /* time */
};

typedef ub2    KCITypeCode;
typedef struct KCIDate KCIDate;
typedef struct KCIType KCIType;
typedef struct KCIIter KCIIter;
typedef struct KCIColl KCIColl;
typedef struct KCIString KCIString;
enum KCITypeGetOpt
{
	KCI_TYPEGET_HEADER,  /* load only the header portion of the TDO when getting type */
	KCI_TYPEGET_ALL      /* load all attribute and method descriptors as well */
};
typedef enum KCITypeGetOpt KCITypeGetOpt;
