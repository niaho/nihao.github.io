...
int main(int argc, char *argv[])
{
	...
	/////创建表结构
	OCIStmt *stmt;
	OCIHandleAlloc(p_env, (void **)&stmt, OCI_HTYPE_STMT, 0, 0);
	createTestTable(p_conn,stmt, p_err);	
	/////插入数据
	////多线程插入数据
	{
		long starttime = clock();
		OCIStmt    *p_stmt2[ThreadNUM];
		...
		double  duration = (double)(finishtime - starttime) / ThreadNUM;
		printf("%f seconds(%d)\n", duration, ThreadNUM);
