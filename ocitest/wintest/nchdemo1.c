 ...
 *
   ...
   MODIFIED   (MM/DD/YY)
   wezhu       03/15/10 - fix compilation error on some platform
...
/* begin: ------ get char counts of each column by call OCIParamGet ---- */ 
   ...
   err = OCIParamGet(stmt2p,OCI_HTYPE_STMT,errhp,(void **)&mypard, (ub4)2);
   if (err == OCI_SUCCESS)
      {
   ...
   err = OCIParamGet(stmt2p,OCI_HTYPE_STMT,errhp,(void **)&mypard, (ub4)3);
   if (err == OCI_SUCCESS)
      {
      checkerr(errhp, OCIAttrGet((dvoid *)mypard,(ub4)OCI_DTYPE_PARAM,
        (dvoid *)&char_semantics, (ub4)0, (ub4)OCI_ATTR_CHAR_USED,
  ...
  /* ---- get char count of second column. ---- */
   err = OCIParamGet(stmt2p,OCI_HTYPE_STMT,errhp,(void **)&mypard, (ub4)4);
   if (err == OCI_SUCCESS)
      {
      checkerr(errhp, OCIAttrGet((dvoid *)mypard,(ub4)OCI_DTYPE_PARAM,
        (dvoid *)&char_semantics, (ub4)0, (ub4)OCI_ATTR_CHAR_USED,
